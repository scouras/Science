...
our %CONFIG_ANAL = (
  ...
  queue          => 'opterons',
  #queue          => 'analysis',
  plot_command    => "/users/scouras/code/scouras/plot_dssp",
...
sub Get_Jobs {
  ...
  # DSSP only executes on whole molecules
  ...
  $job->{'e_hbond'    } = $ANAL->{'e_hbond' } || "";
  $job->{'bridges'    } = $ANAL->{'bridges' } ? "bridges"  : "";
  $job->{'extended'   } = $ANAL->{'extended'} ? "extended" : "";
  $job->{'line'       } = "dssp $job->{'e_hbond'} $job->{'bridges'} $job->{'extended'} $REGION->{'index'}";
